              ...
              <Grid item xs={12}>
                <Box m={2}>
                  ...
                  {['STAGED', 'INGESTED'].includes(job.status) && (
                    <>
                      <Button disabled={!existsOnS3} variant="outlined"
                              onClick={() => downloadZip(job.id, job.reference)}>
                    ...
                    <Grid item xs={12}>
                      ...
                      <Grid container>
                        {job.surveyIds.map((id) => (
                          <Chip key={id} style={{ margin: 5 }} label={id} component="a" href={`/data/survey/${id}`}
                                clickable />
                        ))}
                      </Grid>
                    </Grid>
                          ...
                          <TableRow key={log.id}>
                            <TableCell component="th" scope="row" style={{ verticalAlign: 'top' }}>
                              <Typography variant="caption">
                                {new Date(log.eventTime).toLocaleDateString('en-AU') +
                                  ' ' +
                                  new Date(log.eventTime).toLocaleTimeString('en-AU', {
                                    hour: 'numeric',
                                    minute: '2-digit'
                                  })}
                              </Typography>
                            ...
                            <TableCell>
                              ...
                              {log.filterSet && Object.keys(log.filterSet).map((v) => (log.filterSet[v] ?
                                <TableRow key={v}><TableCell>{v}</TableCell><TableCell>{log.filterSet[v]}</TableCell></TableRow> : <></>))}
                            </TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
