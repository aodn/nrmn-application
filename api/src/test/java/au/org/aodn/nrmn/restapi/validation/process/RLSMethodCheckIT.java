...
class RLSMethodCheckIT {
    ...
    void methodMissingBlock2ShouldFail() {
        ...
        assertTrue(response.getErrors().stream().anyMatch(e -> e.getMessage().startsWith("ERZ1/11/09/2020/7 survey incomplete")));
    }
    ...
    void missingM2ShouldFail() {
        ...
        ValidationResponse response = validationProcess.process(job);
        assertTrue(response.getErrors().stream().anyMatch(e -> e.getMessage().startsWith("ERZ1/11/09/2020/7 survey incomplete: missing M2")));
    }
}
