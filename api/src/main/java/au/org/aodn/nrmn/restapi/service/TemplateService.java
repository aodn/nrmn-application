...
public class TemplateService {
  ...
  private void writeM3SpeciesCsv(Writer writer, Collection<ObservableItemRow> observableItems) throws IOException {
    ...
    List<SpeciesWithAttributesCsvRow> species = observableItems.stream().map(s -> SpeciesWithAttributesCsvRow.builder()
        .speciesName(s.getName()).commonName(s.getCommonName()).letterCode(s.getLetterCode()).build())
    ...
    List<List<String>> records = speciesResult.stream().distinct()
        .sorted(Comparator.comparing(SpeciesWithAttributesCsvRow::getSpeciesName)).map(this::getSpeciesAsCsvRecord)
        .collect(toList());
  ...
  public Set<Site> getSitesForTemplate(Collection<Integer> locations, Collection<String> provinces,
    ...
    sites = Stream.concat(sites, locations.stream().flatMap(l -> siteRepository
        .findAll(Example.of(Site.builder().location(Location.builder().locationId(l).build()).build())).stream()));

    sites = Stream.concat(sites,
        states.stream().flatMap(s -> siteRepository.findAll(Example.of(Site.builder().state(s).build())).stream()));
  ...
  public List<SpeciesWithAttributesCsvRow> getSpeciesForTemplate(Integer mode, Collection<Integer> siteIds,
    ...
    List<SpeciesWithAttributesCsvRow> species = observationRepository.getSpeciesAttributesByIds(observableItemIds)
        ...
        .map(s -> SpeciesWithAttributesCsvRow.builder().letterCode(letterCodeMap.get(s.getId()))
            .speciesName(s.getSpeciesName()).commonName(s.getCommonName()).isInvertSized(s.getIsInvertSized())
