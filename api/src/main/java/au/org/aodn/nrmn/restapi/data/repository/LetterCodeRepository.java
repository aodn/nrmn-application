...
public interface LetterCodeRepository extends JpaRepository<Survey, Integer>, JpaSpecificationExecutor<Survey> {
    ...
    @Query(value = "WITH stage0 AS (SELECT COALESCE(obsitem.superseded_by, obsitem.observable_item_name) AS species_name, SUM(obs.measure_value) AS abundance "
            ...
            + "stage1 AS (SELECT oir.observable_item_id, species_name, abundance, {h-schema}abbreviated_species_code(species_name, 3) AS code_len3 FROM stage0 JOIN {h-schema}observable_item_ref oir on oir.observable_item_name=stage0.species_name),"
