 ...
 * we want to allow flexible table join here if needed in the future. The view use another view which is
 * costly and therefore it is expected that this view is using with Pageable. Otherwise, it will take forever to load
 * the whole dataset.
 */
@Entity
@Immutable
...
public class SurveyListView {
    ...
    /**
     * This view is a heavy load view, hence we use lazy to load value if needed.
     */
    @Audited(targetAuditMode = NOT_AUDITED)
    @OneToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "survey_id", referencedColumnName = "survey_id", nullable = false)
    private DiverMethodAggregateView aggregateView;
