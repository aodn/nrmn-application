...
public class RawValidation extends ValidatorHelpers {
        ...
        public Seq<Tuple2<String, BaseRowValidator>> getRowValidators() {
                return Seq.of(Tuple2.of("Site", siteCodeExists), Tuple2.of("Date", new DateFormatValidation()),
                                ...
                                Tuple2.of("Total",
                                                new IntegerFormatValidation(StagedRow::getTotal, "Total",
                                                                Collections.emptyList())),
                                Tuple2.of("Inverts",
                                                new OptionalIntegerFormatValidation(StagedRow::getInverts, "Inverts")),
        ...
        public Seq<Tuple2<String, BaseRowValidator>> getRawValidators(StagedJob job) {
                ...
                val extendedChecks = job.getIsExtendedSize()
                                ? HashMap.<String, BaseRowValidator>fromStream(Stream.of(Tuple2.of("IsInvertSizing",
                                                new BooleanFormatValidation(StagedRow::getIsInvertSizing,
                                                                "IsInvertSizing"))))
        ...
        public RowWithValidation<Seq<Tuple2<String, Object>>> validate(StagedRow target,
                ...
                val validation = validators
                                .map(tuple -> tuple._2().valid(target).bimap(Function.identity(),
                                                content -> Seq.of(Tuple2.of(tuple._1(), content))))
                                .stream()
                                .reduce(Validated.valid(Seq.empty()), (v1, v2) -> v1.combine(Monoids.seqConcat(), v2));
        ...
        public StagedRowFormatted toFormat(HashMap<String, Object> values, boolean isExtendedSizing) {
                ...
                Integer survey_num = splitDepth.length == 1 ? new Integer(0)
                                : Integer.parseInt(splitDepth[1]);

                val method = (Integer) values.get("Method").orElseGet(null);
                val block = (Integer) values.get("Block").orElseGet(null);
                val species = Optional.ofNullable((ObservableItem) values.get("Species").orElse(null));

                Optional<UiSpeciesAttributes> mayBeSpeciesAttributes = species.isPresent()
                                ? obsRepo.getSpeciesAttributesById(species.get().getObservableItemId())
                                : Optional.empty();
        ...
        public List<StagedRowFormatted> preValidated(List<StagedRow> targets, StagedJob job) {
                ...
                return targets.stream().flatMap(row -> {
                        val validatedRow = validate(row, validators);
                        val validated = validatedRow.getValid();
                        return validated.map(
                                        seq -> toFormat(seq.toHashMap(Tuple2::_1, Tuple2::_2), job.getIsExtendedSize()))
                                        .stream();
