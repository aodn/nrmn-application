...
//    ObservableItem getWithName(@Param("name") String name);
    ...
    @Query(value = "WITH stage0 AS(SELECT COALESCE(obsitem.superseded_by, obsitem.observable_item_name) AS name, sum(obs.measure_value) AS abundance "
            ...
            + "WHERE m.method_id = :methodId AND site_raw.site_id IN :siteIds "
            + "AND (:methodId != 2 OR (obsitem.class NOT IN ('Ophiuroidea', 'Polyplacophora') AND obsitem.family != 'Pyuridae')) GROUP BY name) "
            + "SELECT oir.observable_item_id AS observableItemId, name, oir.letter_code AS letterCode, oir.common_name AS commonName "
            + "FROM stage0 "
            + "INNER JOIN {h-schema}observable_item_ref oir on oir.observable_item_name=stage0.name", nativeQuery = true)
