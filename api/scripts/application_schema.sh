...
docker run -p 5434:5432 --name migration_db -e POSTGRES_PASSWORD=$PGPASSWORD -d postgis/postgis:9.6-2.5 > /dev/null

# wait for postgres to start listening on port 5434

RETRIES=20

until pg_isready -h localhost -U postgres -p 5434 > /dev/null 2>&1 || [ $RETRIES -eq 0 ]; do
...
psql -h localhost -U postgres -p 5434 -c "create database nrmn_migration template template_postgis" > /dev/null
psql -h localhost -U postgres -p 5434 -d nrmn_migration -f "$PROJECT_DIR/src/main/resources/sql/migration.sql" > /dev/null 2>&1
...
liquibase --changeLogFile=changelog.xml \
          ...
          --url=jdbc:postgresql://localhost:5434/nrmn_migration \
          --username=postgres \
          --password=$PGPASSWORD \
          --defaultSchemaName=nrmn \
          diffChangeLog \
          --referenceUrl=jdbc:postgresql://localhost:5432/nrmn_update \
