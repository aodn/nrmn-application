...
import ObservableItemTemplate from './ObservableItemTemplate';
...
const GenericForm = () => {
  ...
  const wormsForm = useSelector((state) => state.form.wormsForm);
  ...
  let fullTitle = id ? 'Edit ' + entityTitle : `New ` + entityTitle;
  ...
  const formContent = () => {
    if (entitySaved) {
      ...
      if (entityName === 'observableItems') {
        const uiSchema = {
          commonName: {'ui:title': 'Name', 'ui:placeholder': 'Enter Species Name...', 'ui:required': true},
          aphiaRef: {'ui:field': 'relationship', 'ui:title': 'Aphia Record'},
          aphiaRelType: {'ui:field': 'relationship', 'ui:title': 'Relationship'}
        };
        const form = {...editItem, ...wormsForm};
        return (
          <BaseForm
            schema={JSSchema}
            uiSchema={uiSchema}
            ...
            fields={fields}
            formData={form}
            template={ObservableItemTemplate}
          />
        );
      } else {
