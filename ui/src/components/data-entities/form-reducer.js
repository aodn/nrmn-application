  ...
  reducers: {
    ...
    selectedItemsLoaded: (state, action) => {
      ...
      if (!('_embedded' in action.payload)) {
        if (action.payload[0]?.diverId) newOptions['divers'] = action.payload;

        if (action.payload[0]?.siteId) newOptions['siteListItems'] = action.payload;

        if (action.payload[0]?.programId) newOptions['programs'] = action.payload;
